...
void VoronoiUtil::merge3(RoadGraph* roads1, RoadGraph* roads2) {
	...
	for (boost::tie(vi, vend) = boost::vertices(roads1->graph); vi != vend; ++vi) {
		...
		if (!GraphUtil::isConnected(roads1, *vi, root1)) {
	...
	for (boost::tie(vi, vend) = boost::vertices(roads2->graph); vi != vend; ++vi) {
		...
		if (!GraphUtil::isConnected(roads2, *vi, root2)) {
	...
	for (boost::tie(ei, eend) = boost::edges(roads1->graph); ei != eend; ++ei) {
		...
		if (GraphUtil::getNumVertices(roads2) == 0) continue;

		float len = roads1->graph[*ei]->getLength();
		if (!roads1->graph[src]->valid) {
			RoadVertexDesc src2 = GraphUtil::getVertex(roads2, roads1->graph[src]->pt);
			GraphUtil::moveEdge(roads1, *ei, roads2->graph[src2]->pt, roads1->graph[tgt]->pt);
		} else if (!roads1->graph[tgt]->valid) {
			RoadVertexDesc tgt2 = GraphUtil::getVertex(roads2, roads1->graph[tgt]->pt);
			GraphUtil::moveEdge(roads1, *ei, roads1->graph[src]->pt, roads2->graph[tgt2]->pt);
		}
		...
		float tab, tcd;
		QVector2D intPt;
		for (int i = 0; i < roads1->graph[*ei]->polyLine.size() - 1 && !overlapped; i++) {
			RoadEdgeIter ei2, eend2;
			for (boost::tie(ei2, eend2) = boost::edges(roads2->graph); ei2 != eend2 && !overlapped; ++ei2) {
				if (!roads2->graph[*ei2]->valid) continue;

				for (int j = 0; j < roads2->graph[*ei2]->polyLine.size() - 1; j++) {
					if (Util::segmentSegmentIntersectXY(roads1->graph[*ei]->polyLine[i], roads1->graph[*ei]->polyLine[i + 1], roads2->graph[*ei2]->polyLine[j], roads2->graph[*ei2]->polyLine[j + 1], &tab, &tcd, true, intPt)) {
						overlapped = true;
						break;
					}
				}
		...
		if (overlapped) {
			RoadVertexDesc snapDesc = GraphUtil::getVertex(roads2, intPt);
			if (!roads1->graph[src]->valid) {
				GraphUtil::moveEdge(roads1, *ei, roads2->graph[snapDesc]->pt, roads1->graph[tgt]->pt);
			} else if (!roads1->graph[tgt]->valid) {
				GraphUtil::moveEdge(roads1, *ei, roads1->graph[src]->pt, roads2->graph[snapDesc]->pt);
			}
	...
	for (boost::tie(ei, eend) = boost::edges(roads2->graph); ei != eend; ++ei) {
		...
		if (GraphUtil::getNumVertices(roads1) == 0) continue;

		float len = roads2->graph[*ei]->getLength();
		if (!roads2->graph[src]->valid) {
			RoadVertexDesc src2 = GraphUtil::getVertex(roads1, roads2->graph[src]->pt);
			GraphUtil::moveEdge(roads2, *ei, roads1->graph[src2]->pt, roads2->graph[tgt]->pt);
		} else if (!roads2->graph[tgt]->valid) {
			RoadVertexDesc tgt2 = GraphUtil::getVertex(roads1, roads2->graph[tgt]->pt);
			GraphUtil::moveEdge(roads2, *ei, roads2->graph[src]->pt, roads1->graph[tgt2]->pt);
		}
		...
		float tab, tcd;
		QVector2D intPt;
		for (int i = 0; i < roads2->graph[*ei]->polyLine.size() - 1 && !overlapped; i++) {
			RoadEdgeIter ei2, eend2;
			for (boost::tie(ei2, eend2) = boost::edges(roads1->graph); ei2 != eend2 && !overlapped; ++ei2) {
				if (!roads1->graph[*ei2]->valid) continue;

				for (int j = 0; j < roads1->graph[*ei2]->polyLine.size() - 1; j++) {
					if (Util::segmentSegmentIntersectXY(roads2->graph[*ei]->polyLine[i], roads2->graph[*ei]->polyLine[i + 1], roads1->graph[*ei2]->polyLine[j], roads1->graph[*ei2]->polyLine[j + 1], &tab, &tcd, true, intPt)) {
						overlapped = true;
						break;
					}
				}
		...
		// 既存エッジと交差する場合は、交点にスナップ
		if (overlapped) {
			RoadVertexDesc snapDesc = GraphUtil::getVertex(roads1, intPt);
			if (!roads2->graph[src]->valid) {
				GraphUtil::moveEdge(roads2, *ei, roads1->graph[snapDesc]->pt, roads2->graph[tgt]->pt);
			} else if (!roads2->graph[tgt]->valid) {
				GraphUtil::moveEdge(roads2, *ei, roads2->graph[src]->pt, roads1->graph[snapDesc]->pt);
			}
 ...
 * 3) 次に、各頂点について、センター頂点まで到達できない頂点を、全て無効にする。
 * 4) 次に、各エッジについて、両端頂点が共に無効の場合、そのエッジも無効にする。
 * 5) 各エッジについて、両端頂点のどちらかが無効になっている場合、有効な頂点vに最も近い敵陣の頂点v'を探す。
